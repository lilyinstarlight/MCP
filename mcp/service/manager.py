...
class Server(object):
    ...
    def start(self):
        ...
        if mcp.config.container:
            self.proc = subprocess.Popen(['/usr/local/bin/mcp-container-helper', str(self.port), self.prefix, os.path.join('bin', 'armagetronad-dedicated'), '--vardir', os.path.join('/', 'srv', 'var'), '--userdatadir', os.path.join('/', 'srv', 'user'), '--configdir', os.path.join('/', 'srv', 'config'), '--datadir', os.path.join('/', 'srv', 'data')], stdin=subprocess.PIPE, stdout=open(os.path.join(self.prefix, 'server.log'), 'a'), stderr=open(os.path.join(self.prefix, 'error.log'), 'w'), env=mcp.common.env.get_server(), cwd=self.prefix)
        else:
