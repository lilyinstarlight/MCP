...
def pull(source_name):
    ...
    if subprocess.call(['bzr', 'pull', prefix], stdout=mcp.common.cmd.log, stderr=subprocess.STDOUT):
...
def prepare(source_name, dst, revision=None):
    ...
    if revision:
        ...
        if subprocess.call(['bzr', 'revert', '-r' + revision, dst], stdout=mcp.common.cmd.log, stderr=subprocess.STDOUT):
