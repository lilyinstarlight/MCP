...
def prepare(library_name, tmp, dst, revision=None):
    ...
    if subprocess.call([os.path.join(tmp, 'install', 'bin', 'python'), os.path.join('.', 'setup.py'), 'install'], stdout=mcp.common.cmd.log, stderr=subprocess.STDOUT, cwd=os.path.join(tmp, 'build')):
